...
class Link():
    ...
    def __init__(self, bandwidth, delay, queue_size, loss_rate):
        ...
        # transmission delay? time it takes to push packet bits onto link
        self.dl = delay
        self.lr = loss_rate
        self.queue_delay = 0.0
        self.queue_delay_update_time = 0.0
        # time packet spends in routing queues
    ...
    def get_cur_latency(self, event_time):
        # time it takes for data to go from source to dest
        return self.dl + self.get_cur_queue_delay(event_time)

    def packet_enters_link(self, event_time):
        if (random.random() < self.lr):  # packet was lost
            return False
        ...
        # can we think of this as the queue being full?
...
class Network():
    ...
    def run_for_dur(self, dur):
        ...
        while self.cur_time < end_time:
            ...
            print("Got event %s, to link %d, latency %f at time %f" % (
                event_type, next_hop, cur_latency, event_time))
            ...
            if event_type == EVENT_TYPE_SEND:
                if next_hop == 0:
                    ...
                    if sender.can_send_packet():
                        ...
                        push_new_event = True
                    heapq.heappush(
                        self.q, (self.cur_time + (1.0 / sender.rate),
                                 sender,
                                 EVENT_TYPE_SEND,
                                 0,
                                 0.0,
            ...
            if push_new_event:
                heapq.heappush(
                    self.q, (new_event_time,
                             sender,
                             new_event_type,
                             new_next_hop,
                             new_latency,
                             new_dropped))
        # cutoff is currently all the same for links
        # though this code isn't used at all lol
        ...
        return (reward / len(self.senders)) * REWARD_SCALE
...
class Sender():
    ...
    def __eq__(self, other):
        return (self.id == other.id,
                self.starting_rate == other.starting_rate,
    ...
    def __lt__(self, other):
        return (1 / self.rate) < (1 / other.rate)
...
class SimulatedMultipartyEnv(gym.Env):
    ...
    def _get_all_sender_obs(self):
        all_observations = []
        for s in self.senders:
            sender_obs = s.get_obs()
            all_observations.append(sender_obs)
        return np.hstack(all_observations)

    def step(self, actions):
        print("Actions: %s" % str(actions))
        #print(actions)
        for s in self.senders: #len(actions)):
            #print("Updating rate for sender %d" % i)
            action = actions
            s.apply_rate_delta(action[0])
            if USE_CWND:
                s.apply_cwnd_delta(action[1])
        ...
        for s in self.senders:
            ...
            event = {
                ...
                'Reward': reward,
                'Send Rate': sender_mi.get('send rate'),
                'Throughput': sender_mi.get('recv rate'),
                'Latency': sender_mi.get('avg latency'),
                'Loss Rate': sender_mi.get('loss ratio'),
                'Latency Inflation': sender_mi.get('sent latency inflation'),
                'Latency Ratio': sender_mi.get('latency ratio'),
                'Send Ratio': sender_mi.get('send ratio')
            }
            self.event_record["Events"].append(event)
            if event['Latency'] > 0.:
                latencies.append(event['Latency'])
        if len(latencies) != 0:
            print('Average latencies: {0}'.format(latencies))
            self.run_dur = 0.5 * np.max(latencies)
    ...
    def create_new_links_and_senders(self):
        ...
        # right now based on existing code all links have the same parameters
        # - might switch this up
        l1 = Link(bw, lat, queue, loss)
        l2 = Link(bw, lat, queue, loss)
        common = Link(bw, lat, queue, loss)

        s1 = Sender(random.uniform(0.3, 1.5) * bw, [l1, common], 0, self.features,
                    history_len=self.history_len)
        s2 = Sender(random.uniform(0.3, 1.5) * bw, [l2, common], 0, self.features,
                    history_len=self.history_len)
        self.senders = [s1, s2]
        self.links = [l1, l2, common]
...
register(id='PccMs-v0', entry_point='network_sim:SimulatedMultipartyEnv')
